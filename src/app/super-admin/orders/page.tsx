  ...
  const handleFastForward = async (orderId: string) => {
    ...
    try {
      const response = await fetch(`/api/orders/${orderId}/fast-forward`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reason: 'Fast-forwarded by super admin' }),
      });
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Complete orders management with call and delivery workflow control
          </p>
        </div>
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4">
                          ...
                          {order.callAssignedBy && (
                            <div className="text-xs text-gray-500 mt-1">
                              Assigned by {order.callAssignedBy.name}
                            </div>
                          )}
                          {order.callNotes && (
